...
import { FileText, Upload, CheckCircle2, Clock, Loader2 } from 'lucide-react'
import Link from 'next/link'
import { useState, useEffect } from 'react'

export default function DashboardPage() {
  const { user } = useAuth()
  const [stats, setStats] = useState<{ plantas_arquitectonicas_procesadas: number; last_updated: string } | null>(null)
              ...
              <CardContent>
                <div className="text-2xl font-bold flex items-center gap-2">
                  {isLoadingStats ? (
                    ...
                    stats?.plantas_arquitectonicas_procesadas ?? 0
                  )}
                </div>
                <p className="text-xs text-muted-foreground">
                  {stats?.last_updated ? `Última: ${new Date(stats.last_updated).toLocaleDateString()}` : 'Sin datos recientes'}
                </p>
              </CardContent>
            </Card>
